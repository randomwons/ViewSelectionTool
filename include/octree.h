...
public:
    ...
    __device__ int traverse(Ray ray, Node* nodes) {
        ...
        if(fmax(fmax(tx0, ty0), tz0) < fmin(fmin(tx1, ty1), tz1)) {
            ...
            int foundNode = -1;
            Stack stack;
            foundNode = traverseNewNode(tx0, ty0, tz0, tx1, ty1, tz1, 0, stack, nodes);
            while (!stack.isEmpty() && foundNode == -1) {
                Stack::SubtreeData* data = stack.top();
                foundNode = traverseChildNodes(data, a, stack, nodes);
            }
            return foundNode;
        }
    ...
    __host__ __device__ int traverseChildNodes(Stack::SubtreeData* data, const unsigned char& a, Stack& stack, const Node* nodes) {

        switch (data->nextQuadrant) {
        case 0:
            data->nextQuadrant = getNextQuadrant(data->txm, 4, data->tym, 2, data->tzm, 1);
            return traverseNewNode(data->tx0, data->ty0, data->tz0, data->txm, data->tym, data->tzm, nodes[data->nodeIdx].firstChildIdx + a, stack, nodes);
        case 1:
            data->nextQuadrant = getNextQuadrant(data->txm, 5, data->tym, 3, data->tz1, 8);
            return traverseNewNode(data->tx0, data->ty0, data->tzm, data->txm, data->tym, data->tz1, nodes[data->nodeIdx].firstChildIdx + (1 ^ a), stack, nodes);
        case 2:
            data->nextQuadrant = getNextQuadrant(data->txm, 6, data->ty1, 8, data->tzm, 3);
            return traverseNewNode(data->tx0, data->tym, data->tz0, data->txm, data->ty1, data->tzm, nodes[data->nodeIdx].firstChildIdx + (2 ^ a), stack, nodes);
        case 3:
            data->nextQuadrant = getNextQuadrant(data->txm, 7, data->ty1, 8, data->tz1, 8);
            return traverseNewNode(data->tx0, data->tym, data->tzm, data->txm, data->ty1, data->tz1, nodes[data->nodeIdx].firstChildIdx + (3 ^ a), stack, nodes);
        case 4:
            data->nextQuadrant = getNextQuadrant(data->tx1, 8, data->tym, 6, data->tzm, 5);
            return traverseNewNode(data->txm, data->ty0, data->tz0, data->tx1, data->tym, data->tzm, nodes[data->nodeIdx].firstChildIdx + (4 ^ a), stack, nodes);
        case 5:
            data->nextQuadrant = getNextQuadrant(data->tx1, 8, data->tym, 7, data->tz1, 8);
            return traverseNewNode(data->txm, data->ty0, data->tzm, data->tx1, data->tym, data->tz1, nodes[data->nodeIdx].firstChildIdx + (5 ^ a), stack, nodes);
        case 6:
            data->nextQuadrant = getNextQuadrant(data->tx1, 8, data->ty1, 8, data->tzm, 7);
            return traverseNewNode(data->txm, data->tym, data->tz0, data->tx1, data->ty1, data->tzm, nodes[data->nodeIdx].firstChildIdx + (6 ^ a), stack, nodes);
        case 7:
            data->nextQuadrant = 8;
            return traverseNewNode(data->txm, data->tym, data->tzm, data->tx1, data->ty1, data->tz1, nodes[data->nodeIdx].firstChildIdx + (7 ^ a), stack, nodes);
        case 8:
            stack.pop();
            return -1;
        }
    ...
    __host__ __device__ int traverseNewNode(const float& tx0, const float& ty0, const float& tz0, const float& tx1, const float& ty1, const float& tz1, const int& nodeIdx, Stack& stack, const Node* nodes) {
